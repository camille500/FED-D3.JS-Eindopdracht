...
function drawChart(error, data) {
    ...
    function update(range) {
        ...
        var subset = chartData.filter(function(d) {
            return d.datum >= new Date(range[0]) && d.datum <= new Date(range[1]);
        });
        ...
        alcohol.enter().append('circle');
        alcohol.exit().remove();

        sigaretten.enter().append('circle');
